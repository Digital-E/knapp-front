...
const Component = () => {
  ...
  useEffect(() => {
    ...
    // if(window.WTCGL === undefined) return

    // if(document.querySelector(`#vertexShader${mode}`) === null) return

    // if(document.querySelector(`#vertexShader${mode}`) === undefined) return
    
    twodWebGL = new WTCGL(
      document.querySelector('canvas#webgl'), 
      document.querySelector(`#vertexShader${mode}`).textContent, 
      document.querySelector(`#fragmentShader${mode}`).textContent,
      window.innerWidth,
      window.innerHeight,
      .5
    );
    ...
    twodWebGL.onRun = (delta) => {
      ...
      let uniform = _ctx.getUniformLocation(twodWebGL._program, `u_buffer`);
    ...
    window.addEventListener('pointermove', (e) => {
      // let ratio = 1;
      // twodWebGL.addUniform('mouse', WTCGL.TYPE_V2, [(e.clientX-512)/1024, (e.clientY-512)/1024*-1]);
      let ratio = window.innerHeight / window.innerWidth;
      if(window.innerHeight > window.innerWidth) {
        mousepos[0] = (e.clientX) / window.innerWidth;
        mousepos[1] = (e.clientY * -1) / window.innerHeight * ratio + ratio;
      } else {
        mousepos[0] = (e.clientX) / window.innerWidth / ratio;
        mousepos[1] = (e.clientY * -1) / window.innerHeight + 1.;
      }
    ...
    loadTextures(textures).then(
      (result) => {
        ...
        document.querySelector(".webgl-background").classList.remove("hide")
        document.querySelector(".webgl-background").classList.add("show")
      },
      (error) => {
        console.log('error');
      }
  ...
  return (
      <canvas className="webgl-background hide" id="webgl" width="500" height="1758"></canvas>
  )
}
